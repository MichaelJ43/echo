...
**Version bump:** `.github/workflows/version-bump.yml` — on merged PR to `main`, bumps **patch**, pushes to `main`, pushes `v*` tag, then runs **`gh workflow run`** to dispatch **Release** (needs **`actions: write`**; `GITHUB_TOKEN` must be passed explicitly via `env: GITHUB_TOKEN: ${{ github.token }}`—a bare `${GITHUB_TOKEN}` in the shell was empty and caused **401**). **workflow_dispatch** bumps **patch**, **minor**, or **major** manually. **Requires** repository secret **`RELEASE_PUSH_TOKEN`** (or **`GH_PAT`**): PAT for `git push` to `main`/tags. Repo **Settings → Actions → Workflow permissions** should allow **read/write** for GITHUB_TOKEN if dispatch fails.
