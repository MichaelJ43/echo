...
| `src-tauri/src/lib.rs` | Tauri builder, plugins (`dialog`, `updater`, `process`), `invoke_handler` for load/save state, HTTP, import/export paths, **`open_external_url`**, **`open_containing_folder`** (reveal workspace file in system file manager), **`list_secret_keys` / `list_secret_logical_names_for_env` / `set_secret` / `delete_secret` / `delete_secrets_for_environment` / `resolve_secret_placeholder_rows`** (OS keychain). |
...
src/                      # React + TS UI, Vite client
  ...
  components/             # TreeNodes, TreeInlineNameRow, UpdatePrompt, HtmlPreviewModal, ImportWorkspaceConfirmDialog, OrphanCredentialsDialog
