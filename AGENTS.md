...
3. **Persistence:** Rust writes JSON to the OS app data directory (`app.path().app_data_dir()` + `collections.json`). Paths surfaced in UI via `get_paths`. **Last HTTP responses** (response panel) and **completion script output** are **session-only** (RAM), keyed by **request id**—switching the active request shows that request’s cached response and script log; nothing is written to disk for responses.
