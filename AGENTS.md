...
3. **Persistence:** Rust writes JSON to the OS app data directory (`app.path().app_data_dir()` + `collections.json`). Paths surfaced in UI via `get_paths`. **Last HTTP responses** (response panel), **completion script output**, and **last Send error** (expand / network / script throw) are **session-only** (RAM), keyed by **request id**—switching the active request shows that request’s cached response, script log, and send error only; nothing is written to disk for responses. Bootstrap load failures use a separate loading-screen error, not the response header.
