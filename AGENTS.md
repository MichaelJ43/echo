...
| `src/components/SecretsDialog.tsx` | Manage local secrets (Collections header context menu). **Planned:** fold into per-environment **Entries** (Variable / File / Secret) in the request panel; table uses narrow kind column + name + value. |
...
2. **Send:** `buildExpandedSendPayload` (`src/lib/expandForSend.ts`) resolves **`{{request:folder/sub/requestName}}`** (entire cached response body) or **`{{request:folder/sub/requestName:dot.path}}`** (value from the body parsed as JSON or YAML via `js-yaml`, then dotted path) against an in-memory **last-response cache** (per request id; not persisted), then **`{{name}}`** from enabled **Variable** and **File** rows in the active environment (`File` rows substitute the path string), then builds `SendRequestPayload` → `send_http_request` (Tauri) or `sendHttpRequestBrowser` (`src/api.ts`). **`{{secret:NAME}}`** is still resolved only in Rust for the desktop app; web build errors if secrets are present. **Secret**-typed environment rows do not participate in `{{name}}` substitution (only `{{secret:name}}` once keychain wiring lands). **Multipart** (`multipart/form-data`) and **binary** (raw bytes from one file) bodies: keys, text values, paths, and content types go through the same `{{var}}` expansion; file bytes are read in Rust from `filePath` / `binaryBody.path` on desktop, or supplied from the browser via base64 session fields (`fileDataBase64` / `browserBase64`) that **`stripEphemeralWorkspaceFields`** (`src/lib/workspacePersist.ts`) removes before save. Rust enforces a **50 MiB** cap per file read for multipart and binary bodies.
...
src/                      # React + TS UI, Vite client
  ...
  lib/                    # variables, collection, requestRef, expandForSend, responseFormat, importWorkspace, workspacePersist, treeNames, treeDraft, scriptRunner, secretPlaceholders, migrateAppState
  types.ts
