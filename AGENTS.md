...
**Version bump:** `.github/workflows/version-bump.yml` — on merged PR to `main`, bumps **patch** and pushes a `v*` tag. **workflow_dispatch** bumps **patch**, **minor**, or **major** manually. **Requires** repository secret **`RELEASE_PUSH_TOKEN`**: a PAT with permission to push to this repo. The default `GITHUB_TOKEN` cannot be used for the push that creates the tag, or GitHub will **not** trigger the Release workflow ([docs](https://docs.github.com/en/actions/security-for-github-actions/security-guides/automatic-token-authentication#using-the-github_token-in-a-workflow)).
