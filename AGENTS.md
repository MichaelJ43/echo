...
| `src-tauri/src/lib.rs` | Tauri builder, plugins (`dialog`, `updater`, `process`), `invoke_handler` for load/save state, HTTP, import/export paths, **`open_external_url`**, **`open_containing_folder`** (reveal workspace file in system file manager), **`list_secret_keys` / `list_secret_logical_names_for_env` / `set_secret` / `delete_secret`** (OS keychain). |
...
| `src-tauri/src/secrets.rs` | OS credential store (`keyring` crate) + `secret_index.json` (key names only, app data dir); composed keys `echo_<uuid>_<logicalName>`. |
