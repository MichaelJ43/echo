...
**Version bump:** `.github/workflows/version-bump.yml` — on merged PR to `main`, bumps **patch** and pushes a `v*` tag. **workflow_dispatch** bumps **patch**, **minor**, or **major** manually. **Requires** repository secret **`RELEASE_PUSH_TOKEN`** (or optional **`GH_PAT`**): a PAT with permission to push to this repo. The job sets `origin` to use that token for `git push` only; checkout uses the default token. The default `GITHUB_TOKEN` cannot be used for pushes that must trigger other workflows ([docs](https://docs.github.com/en/actions/security-for-github-actions/security-guides/automatic-token-authentication#using-the-github_token-in-a-workflow)).
