...
| `src-tauri/src/secrets.rs` | OS credential store (`keyring` crate) + `secret_index.json` (key names only, app data dir); composed keys `echo_<uuid>_<logicalName>`; **deleting an environment** removes all keys with prefix `echo_<thatUuid>_` from the store and index. |
...
2. **Send:** `buildExpandedSendPayload` (`src/lib/expandForSend.ts`) resolves **`{{request:folder/sub/requestName}}`** (entire cached response body) or **`{{request:folder/sub/requestName:dot.path}}`** (value from the body parsed as JSON or YAML via `js-yaml`, then dotted path) against an in-memory **last-response cache** (per request id; not persisted), then **`{{name}}`** from enabled **Variable** and **File** rows in the active environment (`File` rows substitute the path string), then builds `SendRequestPayload` (includes **`environmentId`**) → `send_http_request` (Tauri) or `sendHttpRequestBrowser` (`src/api.ts`). **`{{secret:NAME}}`** is resolved only in Rust for the desktop app using **`echo_<environmentId>_<NAME>`** in the keychain, with **legacy** bare `NAME` keys still read as fallback; web build errors if secrets are present in outbound fields. **Secret**-typed environment **Entries** store values only in the keychain (never in workspace JSON); they do not participate in `{{name}}` substitution—use **`{{secret:NAME}}`** in URLs/headers/bodies. **Multipart** (`multipart/form-data`) and **binary** (raw bytes from one file) bodies: keys, text values, paths, and content types go through the same `{{var}}` expansion; file bytes are read in Rust from `filePath` / `binaryBody.path` on desktop, or supplied from the browser via base64 session fields (`fileDataBase64` / `browserBase64`) that **`stripEphemeralWorkspaceFields`** (`src/lib/workspacePersist.ts`) removes before save. Rust enforces a **50 MiB** cap per file read for multipart and binary bodies.
3. **Persistence:** Rust writes JSON to the OS app data directory (`app.path().app_data_dir()` + `collections.json`). Paths surfaced in UI via `get_paths`. **Deleting an environment** (desktop) invokes **`delete_secrets_for_environment`** so composed keychain keys `echo_<environmentId>_…` are removed with the environment. **Changing an environment entry’s kind** (Variable / File / Secret) clears its workspace `value`; switching **from** Secret also removes that row’s composed (and legacy) keychain entries for the current name (desktop). **Last HTTP responses** (response panel), **completion script output**, **last Send error** (expand / network / script throw), and **Raw vs Pretty** for the response body are **session-only** (RAM), keyed by **request id**—switching the active request shows that request’s cached response, script log, send error, and body view mode only; nothing is written to disk for responses. Bootstrap load failures use a separate loading-screen error, not the response header.
...
src/                      # React + TS UI, Vite client
  ...
  components/             # TreeNodes, TreeInlineNameRow, UpdatePrompt, HtmlPreviewModal, ImportWorkspaceConfirmDialog
  lib/                    # variables, collection, requestRef, expandForSend, responseFormat, importWorkspace, workspacePersist, secretStorageKey, treeNames, treeDraft, scriptRunner, secretPlaceholders, migrateAppState
  types.ts
