...
**Version bump:** `.github/workflows/version-bump.yml` — on merged PR to `main`, bumps **patch**, pushes to `main`, pushes `v*` tag, then **POSTs** `workflow_dispatch` for **Release** (needs **`actions: write`** on `GITHUB_TOKEN`). **workflow_dispatch** bumps **patch**, **minor**, or **major** manually. **Requires** repository secret **`RELEASE_PUSH_TOKEN`** (or **`GH_PAT`**): PAT for `git push` to `main`/tags. Automated tag pushes do not reliably trigger `on: push` workflows; Release is **not** wired to `push.tags`.
